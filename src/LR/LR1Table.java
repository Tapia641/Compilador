...
public class LR1Table extends JFrame {
    ...
    private void createTable() {
        ...
        for (int i = 0; i < m; i++) {
            ...
            //Reducciones            
            HashSet<Pair<LinkedList<Nodo>, HashSet<String>>> endPoints = LRTools.findEndPointsLR1(sn.get(i));
            if (!endPoints.isEmpty()) {
                endPoints.forEach(item -> {
                    int numrule = LRTools.IndexOfRule(item.getKey(), rules);
                    String reduction = "r" + numrule;
                    HashSet<String> symb = item.getValue();
                    symb.forEach(s -> {
                        for (int j = 0; j < n - 1; j++) {
